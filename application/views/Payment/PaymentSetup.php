<div ng-controller="MajorCtrl" class="col-md-10" style="background-color: #ffffff;">
    ...
    <div class="row">
        ...
        <div class="col-md-6 pull-right">
            <input type="number" ng-model="DefaultValueMonthlyPay"  placeholder="Default Value Monthly Pay" class="form-control" />
            <button class="btn btn-primary" ng-click="SetDefaultMonthlyPay()">Set Default</button>
        </div>

    </div>
    <br/>
    <form ng-submit="PaymentSetUpSave()"> 
        <div class="row">
            ...
            <table id="usersTable" class="table table-striped">
                <tr>
                    ...
                    <th>Reg No &nbsp;<a ng-click="sort_with('SMSNotificationNo');"><i class="glyphicon glyphicon-sort"></i></a></th>
...
<!--                    <td> {{User.FacultyName}} </td>
                    ...
                    <td>    <input type="text" required="required"  ng-model="User.MonthlyPay" class="form-control" placeholder="MonthlyPay" name="MonthlyPay" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" /> </td>
                    <td> <input type="text"    ng-model="User.Off" class="form-control" placeholder="Off" name="Off" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />   </td>
                    <td> <input type="text" readonly="readonly" ng-value="User.MonthlyPay-(User.MonthlyPay*(User.Off/100))"  ng-model="User.Total" class="form-control" placeholder="Total Pay" name="" ></td>
                    <td>    <input type="text"   ng-model="User.Others" class="form-control" placeholder="Remarks" name="Others" > </td>
...
<script type="text/javascript">
            ...
            app.controller("MajorCtrl", ["$scope", "$http",
                    ...
                    $scope.PaymentSetUpSave = PaymentSetUpSave;
                            $scope.CheckedUser = [];
                            $scope.MessageUser = '';
...
//                            For Student
                    ...
                    $scope.SetDefaultMonthlyPay = function SetDefaultMonthlyPay() {

                    if ($scope.DefaultValueMonthlyPay > 0)
                    {
                    angular.forEach($scope.Students, function (User)
                    {
                    User.MonthlyPay = $scope.DefaultValueMonthlyPay;
                    });
                    }

                    }

                    //  save or update all pay info
                    function PaymentSetUpSave()
                    {
                    console.log($scope.Students);
                            var r = confirm("If Update the value previous value will be remove. want to update???")

                            if (r == true)
                    {
                    $http({
                    method: 'POST',
                            url: baseUrl + 'Payment/PaymentSetUpSave',
                    ...
                    $scope.Students = [];
                            ...
                            $http({
                            method: 'POST',
                                    url: baseUrl + 'Payment/GetStudent',
                                    headers: {'Content-Type': 'application/json'},
